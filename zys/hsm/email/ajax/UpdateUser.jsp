...
<%
	...
	String mailstatus1 = request.getParameter("mailstatus");;//邮箱状态，1为启用，0为禁用
	String lzsj = request.getParameter("sxsj");//离职时间
	String ygxm = request.getParameter("lzry");//员工姓名
	new BaseBean().writeLog(" 员工编号:" + ygbh);
	JSONObject jsonObject1 = new JSONObject();
	JSONObject Json = new JSONObject();
	String url="http://macom.263.net/api/mail/v2/user/update";	//接口地址
	...
	int[] deptids ={0};
	int mailstatus = Integer.parseInt(mailstatus1);					//邮箱状态，1为启用，0为禁用
	...
	try {
		...
		new BaseBean().writeLog("结果："+result1);
		String errcode = jsonObject.getString("errcode");//返回码， 0代表成功，其他代表失败
		String errmsg = jsonObject.getString("errmsg");//对返回码的文本描述内容
		if(errcode.equals("0")) {
			new BaseBean().writeLog("修改成功，"+xmuserid+"的邮箱状态为："+mailstatus);
			//存入离职员工邮箱表暂存一个月
			String sql ="INSERT INTO uf_yjdsscb (ygmc,ygbh,lzsj) VALUES ('"+ygxm+"','"+xmuserid+"','"+lzsj+"')";
			new BaseBean().writeLog("插入离职员工表sql:"+sql);
			boolean rs2=rs.execute(sql);
			if (rs2){
				jsonObject1.put("code", 200);
				jsonObject1.put("msg","离职成功，保存成功！");
			}
		}else {
			new BaseBean().writeLog("修改失败，错误为："+errmsg);
			jsonObject1.put("code", errcode);
			jsonObject1.put("msg",errmsg);
		}
	} catch (JSONException e) {
		jsonObject1.put("code", 100);
		jsonObject1.put("msg","接口出现问题啦！(> <)");
		e.printStackTrace();
	}
