...
<%
    ...
    String khqcid = request.getParameter("khqc");//	客户全称
    String khfwjd = request.getParameter("khfwjd");//	客户服务阶段
    String khywgzjg = request.getParameter("khywgzjg");//	客户业务归属机构
    String gj = request.getParameter("gj");//	国家地区
    String sf = request.getParameter("sf");//	省份
    String cs = request.getParameter("cs");//	城市
    String hydl = request.getParameter("hydl");//	行业大类
    String hyzl = request.getParameter("hyzl");//	行业中类
    String hyxl = request.getParameter("hyxl");//	行业小类
    ...
    String plgz = request.getParameter("plgz");//	客户品类归属
    String cjrssjg = request.getParameter("cjrssjg");//	创建人所属机构
    String cjrssbm = request.getParameter("cjrssbm");//	创建人所属部门
    String gsgm = request.getParameter("gsgm");//	公司规模
    String rymc = request.getParameter("rymc");//	人员名称
    ...
    try {
        ...
        for (int i= 0;i<khfwjd1.length;i++){//服务阶段
            ...
            rs3.execute(sql3);
            while (rs3.next()){
                khqcid =rs3.getString("id");
                sql4="select count(*) as lxrsl from uf_xsk_dt1 where mainid= "+khqcid+" and fstgyj=0" ;//客户下的联系人数量（发送推广邮件状态为：是）
              //  new BaseBean().writeLog("sql4："+sql4);
                rs4.execute(sql4);
                while (rs4.next()){
                    lxrsl = rs4.getString("lxrsl");
                    lxrsl+=lxrsl;
                }

            }
            new BaseBean().writeLog("lxrsl："+lxrsl);
            sql5= "insert into formtable_main_804 (yjfskhsl) value ("+lxrsl+")";
